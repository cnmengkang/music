  ...
  <div>
    ...
    >
      ...
      <el-table-column label="头像" width="76">
        <template slot-scope="scope">
          <el-image
            fit="cover"
            :width="size"
            :src="scope.row.album.blurPicUrl + '?param=56y56'"
            lazy
          >
          ...
          <span
            class="iconFont icon-play-red align-item-center red border-r-20"
            @click="getCurrentMusicId(scope.row)"
          ></span>
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip label="标题" class-name="title">
        <template slot-scope="scope">
          <div class="eli" :title="scope.row.name">
            ...
            <span v-if="scope.row.alias.length" class="alias ml-5"
      ...
      <el-table-column show-overflow-tooltip label="名字" width="150">
        <template slot-scope="scope">
          <span v-for="(item, index) in scope.row.album.artists" :key="index">{{
            item.name
          }}</span>
        </template>
      </el-table-column>
